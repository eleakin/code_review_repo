/* ===== MODIFIED COPY (auto) 2026-07-02 =====
   Split-year run: HOSPITAL sections -> 2025 (ed2025/ip2025 present), DEATHS -> 2024
   (finalized death_2025 not yet available; only death_2025_preliminary).
   Changes: end_year=25; new death_end_year=24 used in the DEATH section only.
   Review before running on the SAS server (Excel DDE output). Original file unchanged.
   ===== */
...
%let end_year = 25;
%let death_end_year = 24; *deaths held at 2024 - finalized death_2025 not yet on server (only death_2025_preliminary); *change this to the last 2 digits of the desired ending year, the dashboard updates with previous year's data when the deaths data finalized before the end of August;
...
%death (&start_year.,&death_end_year.);




data death_all;
	set	d&start_year. - d&death_end_year.;
		where res_st=29;
...
data race_ethnchs_year;
	do year = 20&start_year. to 20&death_end_year.;
